...
- `trust_score` が通常帯である（docs/04 §6 SEC-TRUST-02。`0011` で追加）

最後から2番目の条件は、1人が同じ峠で方位を変えて8連投すると8回「初」が出る、という一番踏まれやすい抜け道を塞ぐもの。

**実装**: `is_first_bonus_eligible(post_id, grain_version_id)`（`0012`）。この戻り値を `record_facet_post` の `p_eligible` に渡す。ここを経由せずに `true` を組み立てると条件が全部外れるので、ingest 実装（T-13）では必ずこの関数を呼ぶこと。落ちた理由は `first_bonus_eligibility()` が jsonb で返すので、ユーザーへの説明にそのまま使える。

**チェックが未実施なら「通過」扱いにしない。** `v_post_integrity.spoofing_clean` は未実施を `true` に倒すのでこの判定には使わない。信頼度も同じで、`post_trust_scores` に行が無い投稿は資格なしとする。どちらも、チェックを走らせない経路を作るだけで条件が外れてしまうため。
...
手順3で対象バージョンを取り違えても配信中のスポットが動かないよう、`upsert_spot_with_identity()` は **`active` / `deprecated` では既存実体の重心と `h3_index` を書き換えない**（`0013`）。新規スポットの作成はどの status でも通す——通常の取り込み（§1.2 の [POI] / [CELL]）は active に対して走るため、ここを止めると ingest が動かなくなる。配信中に許すのは、未命名の実体に名前を埋めることだけ（§1.4 の命名フロー）。

例外ではなく「動かさずに既存の実体を返す」にしているのは、取り込みが投稿ごとのホットパスで、再試行のたびに落ちると1件の失敗が投稿全体の失敗になるため。取り違え自体は `v_grain_health` の実体数の差で見つける。
